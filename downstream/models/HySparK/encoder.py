...
def sp_in_forward(self, x: torch.Tensor):
    ii = _get_active_ex_or_ii(H=x.shape[2], W=x.shape[3], D=x.shape[4], returning_active_ex=False)
    bhwdc = x.permute(0, 2, 3, 4, 1)
    cn = bhwdc[ii].permute(1, 0)  # select the features on non-masked positions to form a flatten feature `nc` [17787, 3]
    C, N = cn.shape
...
class SparseConvNeXtBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=7, exp_r=4, do_res=False, drop_path=0., layer_scale_init_value=1e-6, sparse=True):
        super().__init__()

        self.do_res = do_res
        self.dwconv = nn.Conv3d(in_channels, in_channels, kernel_size=kernel_size, padding=kernel_size//2, groups=in_channels)  # depthwise conv
...
class SparseEncoder(nn.Module):
    ...
    def dense_model_to_sparse(m: nn.Module, verbose=False, sbn=False):
        ...
        if isinstance(m, nn.Conv3d):
            ...
            m: nn.InstanceNorm3d
            oup = SparseInstanceNorm3d(m.num_features, eps=m.eps, momentum=m.momentum, affine=m.affine, track_running_stats=m.track_running_stats)
            if hasattr(m, "qconfig"):
                oup.qconfig = m.qconfig
