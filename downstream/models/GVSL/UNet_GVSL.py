...
from downstream.models.GVSL.STN import SpatialTransformer, AffineTransformer
import numpy as np
...
class UNet3D_GVSL(nn.Module):
    def __init__(self, pretrained_dir, n_channels=1, n_classes=1, chs=(32, 64, 128, 256, 512, 256, 128, 64, 32)):
        super(UNet3D_GVSL, self).__init__()
        self.n_channels = n_channels

        self.unet_pre = GVSL(n_channels=n_channels)
        if n_channels == 1:
            ...
            for key in state_dict.keys():
                if "unet.inc.double_conv.0"  not in key :
                    unParalled_state_dict[key] = state_dict[key]
            self.unet_pre.load_state_dict(unParalled_state_dict, strict=False)
        
        print("Using pretrained self-supervised GVSL_epoch_1000 !")
